...
class WdlBlock(
  ...
  @tailrec
  private def buildChildrenReversed(
      blocks: List[Block],
      child: ASTNode,
      indent: Indent
  ): List[Block] = {
    if (child == null) {
      ...
      if (child.getElementType == TokenType.WHITE_SPACE) {
        ...
        this.getNode.getElementType == WdlTypes.CALL_INPUT && child.getElementType == WdlTypes.MAPPING
      ) {
  ...
  @Nullable def getSpacing(
      @Nullable child1: Block,
      @NotNull child2: Block
  ): Spacing =
    spacingBuilder.getSpacing(this, child1, child2)

  def isLeaf: Boolean = myNode.getFirstChildNode == null

  private val BLOCK_STARTING_ELEMENTS =
    Set(WdlTypes.LBRACE, WdlTypes.COMMAND_DELIMITER_OPEN)
  private val BLOCK_CLOSING_ELEMENTS =
    Set(WdlTypes.RBRACE, WdlTypes.COMMAND_DELIMITER_CLOSE)
